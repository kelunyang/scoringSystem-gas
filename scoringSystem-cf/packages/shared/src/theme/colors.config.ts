 ...
 * ```
 */
export function getStageColor(status: StageDisplayStatus | string | undefined): string {
  if (!status || !(status in STAGE_COLORS)) {
    return STAGE_COLORS.completed.background
  }
  return STAGE_COLORS[status as StageDisplayStatus].background
}
 ...
 * ```
 */
export function getStageTextColor(status: StageDisplayStatus | string | undefined): string {
  if (!status || !(status in STAGE_COLORS)) {
    return STAGE_COLORS.completed.text
  }
  return STAGE_COLORS[status as StageDisplayStatus].text
}
 ...
 * ```
 */
export function getStageColorConfig(status: StageDisplayStatus | string | undefined): StageColorConfig {
  if (!status || !(status in STAGE_COLORS)) {
    return STAGE_COLORS.completed
  }
  return STAGE_COLORS[status as StageDisplayStatus]
}
 ...
 * ```
 */
export function getStageGradient(status: StageDisplayStatus | string | undefined): string {
  if (!status || !(status in STAGE_COLORS)) {
  ...
  const config = STAGE_COLORS[status as StageDisplayStatus]
  return `linear-gradient(90deg, ${config.background} 0%, ${config.gradient.start} 15%, ${config.gradient.end} 85%, ${config.background} 100%)`
}
 ...
 */
export function isWCAGCompliant(status: StageDisplayStatus): boolean {
 ...
 * @returns 階段顯示狀態數組
 */
export function getAllStageStatuses(): StageDisplayStatus[] {
  return Object.keys(STAGE_COLORS) as StageDisplayStatus[]
}
 ...
 * ```
 */
export function getButtonColor(type: ButtonColorType | string | undefined): string {
  if (!type || !(type in BUTTON_COLORS)) {
    return BUTTON_COLORS.neutral.background
  }
  return BUTTON_COLORS[type as ButtonColorType].background
}
 ...
 * ```
 */
export function getButtonTextColor(type: ButtonColorType | string | undefined): string {
  if (!type || !(type in BUTTON_COLORS)) {
    return BUTTON_COLORS.neutral.text
  }
  return BUTTON_COLORS[type as ButtonColorType].text
}
 ...
 * ```
 */
export function getButtonHoverColor(type: ButtonColorType | string | undefined): string {
  if (!type || !(type in BUTTON_COLORS)) {
    return BUTTON_COLORS.neutral.hover
  }
  return BUTTON_COLORS[type as ButtonColorType].hover
}
 ...
 * ```
 */
export function getButtonColorConfig(type: ButtonColorType | string | undefined): ButtonColorConfig {
  if (!type || !(type in BUTTON_COLORS)) {
    return BUTTON_COLORS.neutral
  }
  return BUTTON_COLORS[type as ButtonColorType]
}
 ...
 * @returns 是否符合 WCAG AA 標準 (對比度 >= 4.5:1)
 */
export function isButtonWCAGCompliant(type: ButtonColorType): boolean {
 ...
 * @returns 按鈕顏色類型數組
 */
export function getAllButtonTypes(): ButtonColorType[] {
  return Object.keys(BUTTON_COLORS) as ButtonColorType[]
}
