...
/** 時間值：Date 物件、ISO 字串或 timestamp */
type TimeValue = Date | string | number

/**
 * 階段數據
 * - extraTime: (可選) 延長時間，timestamp 或 Infinity（手動關閉）
 * - extraTimeText: (可選) 延長段顯示文字
 */
export interface GanttStage {
  ...
  status: string
  extraTime?: number
  extraTimeText?: string
}

/** 預處理後的階段（缺失時間已填充並標記） */
export interface ProcessedGanttStage extends GanttStage {
  startTime: TimeValue
  endTime: TimeValue
...
/** viewport-change 事件 payload */
export interface ViewportChangePayload {
  viewport: { start: Date | null; end: Date | null }
  fullTimeRange: { start: Date | null; end: Date | null }
  xScale: { domain: [Date | null, Date | null] }
}

interface BarDatum {
  type: 'solid' | 'extra'
  stage: ProcessedGanttStage
  index: number
  startDate: Date
  endDate: Date
  label: string
...
const props = withDefaults(defineProps<{
  ...
  /** 是否啟用拖曳（橫向拖曳 + 慣性滾動 + 滾輪縮放）；false 為固定視圖 */
  enableDrag?: boolean
  /** 是否顯示 Mini-map 時間感知條（僅 enableDrag=true 時有效） */
  showMinimap?: boolean
  /** 固定模式下顯示的階段數量（僅 enableDrag=false 時生效） */
  fixedStageCount?: number
  /** 圖表基礎高度（像素）；動態模式依階段數量自動計算 */
  height?: number
  /** 緊湊模式（卡片等空間受限場景） */
  compact?: boolean
  /** 初始置中時間點；undefined 為自動判定（現在時間或最後階段） */
  centerTime?: TimeValue
}>(), {
  milestones: () => [],
  enableDrag: true,
  showMinimap: true,
...
const emit = defineEmits<{
  'stage-click': [stage: ProcessedGanttStage]
  'viewport-change': [payload: ViewportChangePayload]
}>()
...
// 完整時間範圍
const fullTimeRange = ref<{ start: Date | null; end: Date | null }>({
  start: null,
  end: null
})

// 當前可視窗口
const viewport = ref<{ start: Date | null; end: Date | null }>({
  start: null,
  end: null
})
...
// 可見階段列表
const visibleStages = ref<ProcessedGanttStage[]>([])

// 拖曳狀態
const isDragging = ref(false)
const dragStartX = ref(0)
const dragStartViewport = ref<{ start: Date; end: Date } | null>(null)
...
// D3 比例尺和軸
const xScale = shallowRef<d3.ScaleTime<number, number> | null>(null)
const yScale = shallowRef<d3.ScaleBand<number> | null>(null)
const svg = shallowRef<d3.Selection<SVGSVGElement, unknown, null, undefined> | null>(null)
const g = shallowRef<d3.Selection<SVGGElement, unknown, null, undefined> | null>(null)

// Minimap
const minimapBar = ref<HTMLDivElement | null>(null)

// Tooltip
const tooltip = shallowRef<ReturnType<typeof createTooltip> | null>(null)

// Pattern 定義標記（避免重複創建）
let patternDefined = false

// Event type 顏色映射（使用 Map 緩存）
const eventTypeColors = ref(new Map<string, string>())
 ...
 */
const preprocessStages = (): ProcessedGanttStage[] => {
  ...
  return validStages.map(stage => {
    ...
    return {
      ...stage,
      startTime: stage.startTime ?? earliestStart.toDate(),
      endTime: stage.endTime ?? latestEnd.toDate(),
...
// 預處理後的階段數據
const processedStages = ref<ProcessedGanttStage[]>([])
...
const calculateDynamicViewportDays = () => {
  ...
  const totalMs = fullTimeRange.value.end!.getTime() - fullTimeRange.value.start!.getTime()
...
const renderChart = () => {
  ...
  // Y 軸比例尺（階段）
  yScale.value = d3.scaleBand<number>()
...
const renderXAxis = (width: number, height: number) => {
  // 固定使用 1 天間隔
  ...
  // 添加 viewport 時間資訊
  const viewportDuration = viewport.value.end!.getTime() - viewport.value.start!.getTime()
...
const renderReferenceLines = (height: number) => {
 ...
 */
const renderLegend = (width: number) => {
...
const renderBarRectangles = (barsData: BarDatum[]) => {
  g.value!.selectAll<SVGRectElement, BarDatum>('.stage-bar, .stage-bar-extra')
    .data(barsData)
    .enter()
    .append('rect')
    ...
    .attr('y', d => yScale.value!(d.index) ?? 0)
...
const renderBarLabels = (barsData: BarDatum[]) => {
  // 過濾掉沒有 label 的 bar（extraTimeText 為空字串時）
  const labelsData = barsData.filter(d => d.label && d.label.trim() !== '')

  g.value!.selectAll<SVGTextElement, BarDatum>('.stage-label, .stage-label-extra')
    .data(labelsData)
    .enter()
    .append('text')
    ...
    .attr('y', d => (yScale.value!(d.index) ?? 0) + yScale.value!.bandwidth() / 2)
    .attr('text-anchor', d => {
      const barStartX = xScale.value!(d.startDate)
      const barEndX = xScale.value!(d.endDate)
      const viewportWidth = xScale.value!.range()[1] - xScale.value!.range()[0]
...
const showTooltip = (event: MouseEvent, stage: ProcessedGanttStage, _barType: 'solid' | 'extra' = 'solid') => {
  // 拖曳模式下不顯示 tooltip
  ...
  const statusText = ({
    ...
    voting: '投票中',
    completed: '已完成'
  } as Record<string, string>)[stage.status] || stage.status
...
const onDragStart = (event: d3.D3DragEvent<SVGSVGElement, unknown, unknown>) => {
...
const onDrag = (event: d3.D3DragEvent<SVGSVGElement, unknown, unknown>) => {
...
const setupMinimapDrag = () => {
  ...
  const onMinimapMouseMove = (event: MouseEvent) => {
