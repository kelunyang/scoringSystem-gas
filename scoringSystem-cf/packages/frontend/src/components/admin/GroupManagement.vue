...
import type { MemberWithChange } from './group-management/shared/BatchUpdateRoleDrawer.vue'
...
// 路由以 props 形式傳入（本組件實際自 route.params 取值，宣告保留 API 相容）
...
const registerRefresh = inject<(fn: (() => Promise<void> | void) | null) => void>('registerRefresh', () => {})

// Register action function with parent SystemAdmin
const registerAction = inject<(fn: (() => Promise<void> | void) | null) => void>('registerAction', () => {})

// Get activeTab from route meta or default to 'global'
const activeTab = computed(() => {
  return (route.meta?.groupType as string) || 'global'
})
...
const groups = shallowRef<ProjectGroup[]>([])
const globalGroups = shallowRef<GlobalGroup[]>([])
...
const selectedGroup = ref<ProjectGroup | null>(null)
...
const pendingRemoveMembers = ref<RemovableMember[]>([])
const pendingRemoveGroup = ref<ProjectGroup | GlobalGroup | null>(null)
const removingMembers = ref(false)
const pendingRemoveType = ref<'' | 'project' | 'global'>('') // 'project' or 'global'
const editingGroupData = ref(false)
const updatingGroupId = ref<string | null>(null) // Track which group's allowChange is being updated

// Deactivate group drawer state
const showDeactivateDrawer = ref(false)
const pendingDeactivateGroup = ref<ProjectGroup | null>(null)
...
const pendingUpdateMembers = ref<MemberWithChange[]>([])
const pendingUpdateGroup = ref<ProjectGroup | null>(null)
const updatingRoles = ref(false)
const updatingMemberEmail = ref<string | null>(null) // Track which member's role is being updated
const pendingRoleChanges = ref(new Map<string, MemberRole>()) // Track pending role changes for batch mode
...
const selectedProjectForViewers = ref<Project | null>(null)
...
} = useExpandable<GroupMember[]>({ singleMode: true })
...
const addingMemberForProjectGroup = ref<string | null>(null)
const addingMemberForGlobalGroup = ref<string | null>(null)
...
const selectedProjectGroups = reactive(new Set<string>())
const selectedGlobalGroups = reactive(new Set<string>())
...
const memberForm = reactive<{ userEmail: string; role: MemberRole }>({
  userEmail: '',
  role: 'member'
})
...
const loadProjectGroups = async () => {
  ...
  try {
    ...
    handleError(error instanceof Error ? error : String(error), { action: '載入專案群組' })
  } finally {
    loading.value = false
  }
}
...
const editGroup = async (group: ProjectGroup) => {
  try {
    ...
    editingGroupData.value = false
    handleError(error instanceof Error ? error : String(error), { action: '載入群組資料' })
...
const handleEditDrawerClose = (done: () => void) => {
  cancelEditGroup()
  done()
}

const deactivateGroup = async (group: ProjectGroup) => {
...
const confirmDeactivateGroup = async () => {
  ...
  try {
    ...
    handleError(error instanceof Error ? error : String(error), { action: '停用群組' })
  } finally {
...
const activateGroup = async (group: ProjectGroup) => {
  try {
    ...
    handleError(error instanceof Error ? error : String(error), { action: '啟用群組' })
  }
}

const updateGroupAllowChange = async ({ groupId, allowChange }: { groupId: string; allowChange: boolean }) => {
  try {
    ...
    handleError(error instanceof Error ? error : String(error), { action: '更新群組設定' })
  } finally {
...
const createBatchGroups = async (payload: { groupCount: number; allowChange: boolean }) => {
...
const saveProjectGroup = async () => {
  ...
  try {
    ...
    handleError(error instanceof Error ? error : String(error), { action: '保存專案群組' })
  } finally {
    updating.value = false
  }
}
...
const loadGlobalGroups = async () => {
  try {
    ...
    handleError(error instanceof Error ? error : String(error), { action: '載入全域群組' })
  } finally {
    loading.value = false
  }
}
...
const editGlobalGroup = (group: GlobalGroup) => {
...
const saveGlobalGroup = async () => {
  ...
  try {
    ...
    handleError(error instanceof Error ? error : String(error), { action: '保存全域群組' })
  } finally {
    savingGlobalGroup.value = false
  }
}

const deactivateGlobalGroup = async (group: GlobalGroup) => {
  // 使用 TanStack Query mutation
...
const activateGlobalGroup = async (group: GlobalGroup) => {
...
const toggleGlobalGroupStatus = async (group: GlobalGroup) => {
  try {
...
const toggleProjectGroupSelection = (groupId: string) => {
...
const toggleGlobalGroupSelection = (groupId: string) => {
...
const batchDeactivateGlobalGroups = async () => {
  ...
  try {
    ...
    if (response.success) {
      const successCount = (response.data as { successCount?: number } | undefined)?.successCount || selectedGlobalGroups.size
...
const toggleProjectGroupExpansion = async (group: ProjectGroup) => {
...
const toggleGlobalGroupExpansion = async (group: GlobalGroup) => {
  const groupId = group.groupId
...
const loadGlobalGroupMembersInline = async (groupId: string) => {
  // Note: Loading state is now managed by useExpandable composable
  try {
...
const openProjectAddMemberForm = async (group: ProjectGroup) => {
...
const openGlobalAddMemberForm = async (group: GlobalGroup) => {
...
const addSelectedMembersToProjectGroup = async (group: ProjectGroup) => {
...
const addSelectedMembersToGlobalGroup = async (group: GlobalGroup) => {
  if (selectedUsersToAdd.value.length === 0) {
    handleError('請至少選擇一個使用者', { type: 'error' })
    return
  }
...
const removeMemberFromProjectGroup = async ({ member, group }: { member: GroupMember; group: ProjectGroup }) => {
  try {
    ...
    handleError(error instanceof Error ? error : String(error), { action: '移除成員' })
  } finally {
    removingMemberEmail.value = ''
  }
}

const removeMemberFromGlobalGroup = async ({ member, group }: { member: GroupMember; group: GlobalGroup }) => {
  try {
    removingMemberEmail.value = member.userEmail
...
const handleBatchRemoveProjectMembers = ({ members, group }: { members: GroupMember[]; group: ProjectGroup }) => {
...
const handleBatchRemoveGlobalMembers = ({ members, group }: { members: GroupMember[]; group: GlobalGroup }) => {
...
const confirmRemoveMembers = async () => {
  if (!pendingRemoveGroup.value) return
  try {
    ...
    handleError(error instanceof Error ? error : String(error), { action: '批量移除成員' })
  } finally {
...
const updateMemberRole = async ({ member, group, newRole }: { member: GroupMember; group: ProjectGroup; newRole: MemberRole }) => {
  try {
    ...
    handleError(error instanceof Error ? error : String(error), { action: '更新成員角色' })
  } finally {
    updatingMemberEmail.value = null
  }
}

const handleRoleChangePending = ({ userEmail, groupId, newRole }: { userEmail: string; groupId: string; newRole: MemberRole }) => {
...
const handleBatchUpdateRoles = ({ members, group }: { members: GroupMember[]; group: ProjectGroup }) => {
  ...
  const membersWithChanges = members.map((member): MemberWithChange => {
    const oldRole = member.role ?? 'member' // Original role from database
...
const confirmBatchUpdateRoles = async (updates: Array<{ userEmail: string; newRole: MemberRole }>) => {
  if (!pendingUpdateGroup.value) return
  try {
    ...
    handleError(error instanceof Error ? error : String(error), { action: '批量更新角色' })
  } finally {
...
const loadProjectViewers = async (projectId: string) => {
  ...
  try {
    ...
    projectViewers.value = []
    handleError(error instanceof Error ? error : String(error), { action: '載入存取者清單' })
  } finally {
...
const searchUsers = async (payload: { searchText: string; role: ViewerRole } | string) => {
  const searchText = typeof payload === 'string' ? payload : (payload?.searchText || '')
  ...
  try {
    ...
    searchResults.value = []
    handleError(error instanceof Error ? error : String(error), { action: '搜尋使用者' })
  } finally {
    searchingUsers.value = false
  }
}

const addSelectedViewers = async (payload: { users: SelectedUser[]; role: ViewerRole }) => {
  ...
  try {
    ...
    handleError(error instanceof Error ? error : String(error), { action: '批次新增存取者' })
  } finally {
    loadingViewers.value = false
  }
}
...
const updateViewerRole = async ({ userEmail, newRole }: { userEmail: string; newRole: ViewerRole }) => {
  ...
  try {
    ...
    handleError(error instanceof Error ? error : String(error), { action: '更新角色' })
  }
}
...
const removeViewer = async (userEmail: string) => {
  ...
  try {
    ...
    handleError(error instanceof Error ? error : String(error), { action: '移除存取者' })
  }
}
...
const batchUpdateRoles = async () => {
  ...
  try {
    ...
    console.error('Error batch updating roles:', error)
    handleError(error instanceof Error ? error : String(error), { action: '批次轉換角色' })
  } finally {
    loadingViewers.value = false
  }
}
...
const batchRemoveViewers = async () => {
  ...
  try {
    ...
    handleError(error instanceof Error ? error : String(error), { action: '批次刪除存取者' })
  } finally {
    loadingViewers.value = false
  }
}
...
const loadProjects = async () => {
  try {
    ...
    handleError(error instanceof Error ? error : String(error), { action: '載入專案清單' })
  } finally {
