...
---
## 2026-09-07 ｜ 「lint 0 error」不等於沒有 `any`，自己寫的正則又把數字量成 0

**症狀**：連續三次回報「`any` 已歸零」，實際上前端還有 66 處。

**兩個獨立的錯誤疊在一起：**

**一、規則是 `warn` 不是 `error`。**
`eslint.config.js` 裡 `@typescript-eslint/no-explicit-any` 設成 `'warn'`
（原本就有 `TODO(lint-debt)` 說明要逐步清理後升回 error）。
所以 `pnpm lint` 顯示「0 error」時，底下可能有幾百個 any warning。
**把「lint 過」當成「沒有 any」是錯的。**

**二、為了「修正」量法而自己寫的正則是壞的。**

```js
// 壞的：幾乎只抓得到 ` as any`
/(?<![\w$])(:\s*any\b|<any>|as any\b)/
```

`(?<![\w$])` 是套用在整個 group 之前的，等於要求 `:` 前面不能是文字
字元——但 `walletData: any` 的冒號前正是 `a`，所以匹配直接失敗。
同理 `(x: any)`、`Record<string, any>`、`<T = any>`、
`UseMutationReturnType<any, ...>` 全部漏掉。

它回報 348 → 147 → 0，看起來像是穩定收斂，其實是量錯的量尺在自我印證。
更糟的是：我拿這個壞數字去「更正」上一輪那個比較寬鬆但**量級正確**的
grep（888/382），把對的說成錯的。真實起點是 820。

**教訓與防護**：
1. **有現成工具就別自己寫量尺。** ESLint 已經有 `no-explicit-any`，
   `pnpm exec eslint <path> | grep -c "no-explicit-any"` 一行就夠。
2. **自製的量尺一定要先校準。** 拿它去量一個「已知答案」的目標，
   對得上才能用。這次事後用 ESLint 校準（0/0/66 完全吻合）才敢採信。
3. **收斂到 0 的數字要特別懷疑。** 量尺壞掉最常見的表現就是回報 0；
   「歸零」應該觸發複驗，而不是慶祝。
4. 規則設成 `warn` 的清理型任務，驗收條件要明講是「warning 數」，
   不能只寫「lint 通過」。
