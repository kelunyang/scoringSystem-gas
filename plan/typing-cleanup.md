...
### 批次 4b：frontend API 邊界（未做，381 處，48 檔）

**開工前先讀 [issue.md](issue.md) #011。** 2026-09-06 已做完實測，
結論是：`app.route()` 與各 router 的 `.get()/.post()` 全部寫成獨立
述句而非串接，所以 `typeof app` 的 route schema 永遠是空的——
和 `.d.ts`、和 monorepo 設定都無關。要走 Hono 原生推導得改寫
21 個 router、246 個路由註冊。#011 裡有三條路的取捨表。

**在 #011 有結論之前不要動這 381 處。** 手寫型別去描述一個沒人
驗證的契約，和批次 1 刪掉的 `types/api.ts` 是同一種東西。
