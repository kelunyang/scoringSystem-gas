...
- **收尾清理**（2026-07-08，`04c9ffe`）：全量複掃確認零 Options API 殘留（150/150，含新增組件；defineComponent/mixins/this.$ 皆零命中）；順手刪除無引用的 `GlobalAuthModal.new.vue`、`StageTimeline.refactored.backup.vue`，並將現役的 `ProjectDetail-New.vue` 更名為 `ProjectDetail.vue`、`WalletNew.vue` 更名為 `Wallet.vue`（`e57bb00`；router import 同步）
- 執行修正註記：
