      ...
      <table v-else class="tags-table">
        ...
        <tbody>
          ...
          >
            ...
            <td class="create-time">{{ formatTime(tag.createdTime) }}</td>
            <td class="status">
        ...
        <div class="form-group">
          ...
          <div class="color-picker">
            <button 
              ...
              :class="{ selected: tagForm.tagColor === color }"
...
export default {
  ...
  setup() {
      ...
      try {
        if (showCreateModal.value) {
          ...
          if (response.success) {
            ...
            alert('建立標籤失敗: ' + (response.error?.message || '未知錯誤'))
          }
          ...
          if (response.success) {
            ...
            alert('更新標籤失敗: ' + (response.error?.message || '未知錯誤'))
          }
        }
      } catch (error) {
    ...
    const archiveTag = async (tag) => {
      if (!confirm(`確定要封存標籤「${tag.tagName}」嗎？封存後將無法再指派給新的用戶或專案。`)) {
        return
      }
      ...
      try {
        ...
        if (response.success) {
          ...
          alert('封存標籤失敗: ' + (response.error?.message || '未知錯誤'))
        }
      } catch (error) {
    ...
    const restoreTag = async (tag) => {
      try {
        ...
        if (response.success) {
          ...
          alert('還原標籤失敗: ' + (response.error?.message || '未知錯誤'))
        }
      } catch (error) {
