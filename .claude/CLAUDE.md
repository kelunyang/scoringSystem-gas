...
討論中出現以下內容時，**主動落檔**，不要讓它隨對話蒸發：

| 內容性質 | 歸檔位置 |
|----------|----------|
| 未解決的問題、還沒修的洞、待決事項 | [plan/issue.md](plan/issue.md) A 區 |
| 已經有結論、但容易再被重想的設計疑問 | [plan/issue.md](plan/issue.md) B 區（附結論與理由） |
| 已踩過並解決的事故（症狀 → 根因 → 教訓與防護） | [plan/pitfalls.md](plan/pitfalls.md) |

**流轉**：issue.md A 區項目修好後移出；若過程有教訓價值，轉寫成 pitfalls.md 條目。
兩檔皆新項目往上加。

**B 區的用意**：封存「已經查證過、有明確結論」的疑問，避免半年後再問一次同樣的問題。
落檔時務必寫下**理由與證據（檔案路徑＋行號）**，不是只寫結論。

**注意**：`plan/issues_backlog.md.bak` 是 GAS 時代的退役遺物（iframe 沙盒等議題），
勿參考、勿復活、勿寫入。
